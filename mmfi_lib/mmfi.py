...
class MMFi_Dataset(Dataset):
    ...
    def get_data_type(self, mod):
        if mod in ['infra1', "infra2"]:
            return ".npy"
        elif mod in ["lidar", "mmwave"]:
            return ".bin"
        elif mod in ["depth", "rgb"]:
            return ".png"
...
def collate_fn_padd(batch):
    ...
    _keypoints = [np.array(sample['keypoints']) for sample in batch]
    _keypoints = torch.FloatTensor(np.array(_keypoints))
    batch_data['keypoints'] = _keypoints

    for mod in batch_data['modality']:
        if mod in ['mmwave', 'lidar']:
            _input = [torch.Tensor(sample['input_' + mod].copy()) for sample in batch]
